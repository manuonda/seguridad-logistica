...
use App\Models\TramiteVotacionModel;
...
class ConstanciaPorNoVotacion extends BaseController {
    ...
    public function index() {
        $data['id_tipo_tramite'] = TIPO_TRAMITE_CONSTANCIA_POR_NO_VOTACION;
        $tipoTramiteModel = new TipoTramiteModel();
        $tramite = $tipoTramiteModel->find(TIPO_TRAMITE_CONSTANCIA_POR_NO_VOTACION);
        ...
        $time = time();
        $data['turno_user_id'] = TIPO_TRAMITE_CONSTANCIA_POR_NO_VOTACION . date("dmYHis", $time) . rand(1,10000);
    ...
    public function loadData($idTramite) {
        ...
        $tramiteVotacionModel = new TramiteVotacionModel();
        ...
        $tramiteNoVotacion = $tramiteVotacionModel->where('id_tramite', $tramite['id_tramite'])->first();
        ...
        //$data['razon_social'] = $tramite['razon_social'];
        //$data['motivo'] = $tramite['motivo'];
        ...
        //tramite votacion 
        $data['id_tramite_votacion'] = $tramiteNoVotacion['id_tramite_votacion'];
        $data['razon_social'] = $tramiteNoVotacion['razon_social'];
        $data['descripcion'] = $tramiteNoVotacion['descripcion'];
        $data['fecha_votacion'] = $tramiteNoVotacion['fecha_votacion'];
        $data['nombre_eleccion']=$tramiteNoVotacion['nombre_eleccion'];
        $data['id_provincia'] =$tramiteNoVotacion['id_provincia'];
        ...
        $data['razones'] = $this->getRazonesNoVotacion();
...
//         $data['departamentos'] = $departamentoModel->whereIn('id_departamento', [288, 291, 292, 293, 294, 297, 300, 302])->findAll();
        ...
        $data['id_tipo_tramite'] = TIPO_TRAMITE_CONSTANCIA_POR_NO_VOTACION;
        $data['contenidopaso1'] = "constanciaPorNoVotacion";
    ...
    public function cargarForm($data = [] , $tipoForm = "wizard") {
        ...
        $provnciaModel = new ProvinciaModel();
        ...
        $data['razones'] = $this->getRazonesNoVotacion();
        $data['provincias'] = $provnciaModel->findAll();
        ...
        if ($tipoForm == "wizard") {
            ...
            $data['contenidoedit'] = "constanciaPorNoVotacion";

        } else {
            // Verifico si existen tramites anteriores            
            if ( !empty(session()->get('id_rol')) && session()->get('id_rol')==ROL_UNIDAD_ADMINISTRATIVA) {
                $listadoTramites = $this->tramiteModel->getTramiteValidado($data['documento'], $data['id_tramite'], TIPO_TRAMITE_CONSTANCIA_POR_NO_VOTACION);
                if ( sizeof($listadoTramites) > 0 && $data['estado'] != TRAMITE_VALIDADO ) {
                    ...
                    $data['contenidoedit'] = "constanciaPorNoVotacion";
                    $data['contenidoview'] = "constanciaPorNoVotacion_view";
                } else {
                    $data["action"] = "edit";
                    $data["estados"] = $this->get_estados($data["estado"]);
                    $data['contenido'] = "vista";
     ...
     * esta corresponde a las secciones , dependencias
     */
    public function guardar() {
        ...
        $validation->setRules([
            ...
            'razon_social' => ['label' => 'Razon', 'rules' => 'required'],
            'fecha_votacion' => ['label' => 'Fecha Paro Transporte', 'rules' => 'required'],
        ...
        // CONSTANCIA POR NO VOTACION
        $data['id_tramite_votacion'] = $this->request->getVar('id_tramite_votacion');
        $data['razon_social'] = $this->request->getVar('razon_social');
        $data['fecha_votacion'] = $this->request->getVar('fecha_votacion');
        $data['descripcion'] = strtoupper($this->request->getVar('descripcion'));
        $data['nombre_eleccion'] = ELECCIONES_GENERALES;
        $data['id_provincia'] = $this->request->getVar('id_provincia');
        ...
        if ($validation->withRequest($this->request)->run()) {
            ...
            if(empty($data['id_tramite'])) { // insert
                ...
                $id_tramite = $this->tramiteModel->insertarConstanciaPorVotacion($data);
            }else { // update
                $id_tramite = $this->tramiteModel->updateConstanciaPorVotacion($data);
            }
            ...
            if(!empty(session()->get('id_rol')) && session()->get('id_rol')==ROL_UNIDAD_ADMINISTRATIVA) {
                ...
                $data['contenido_paso1'] = "constanciaPorNoVotacion";
                $data['contenido'] = "wizard/wizard";
                $data['action'] = "certificadoResidencia/guardarData";
            }
    ...
    public function guardarData() {
        ...
        $validation->setRules([
            ...
            'email' => ['label' => 'Email', 'rules' => 'required'],
            'razon_social' => ['label' => 'Razon', 'rules' => 'required'],
            'fecha_votacion' => ['label' => 'Fecha Votacion', 'rules' => 'required'],
...
//                     'turno' => $turno,
    ...
    public function descargar($id_tramite) {
        $tramitePersonaModel = new TramitePersonaModel();
        $tramiteNoVotacionModel = new TramiteVotacionModel();
        ...
        $provincia = null;
        $constanciaNoVotacion = $tramiteNoVotacionModel->where('id_tramite', $tramite['id_tramite'])->first();
        ...
        $pdf->AddPage();
        $html = $pdf->getHeaderTramite($tramite['id_tramite']);
        $html = $this->getBodyTramite2($html, $tramite, $titularTramite , $constanciaNoVotacion);
...
//         $imgdata = base64_decode($pdf->imagenConTexto($titularTramite['apellido'].' '.$titularTramite['nombre'].' - '.$tipoDocumento['tipo_documento'].': '.$titularTramite['documento']));
        $imgdata = base64_decode($pdf->imagenConTextoYcoordenadas($titularTramite['apellido'].' '.$titularTramite['nombre'].' - '.$tipoDocumento['tipo_documento'].': '.$titularTramite['documento'], 540, 750));
        $pdf->Image('@'.$imgdata);
        $pdf->SetAlpha(1);
        
        
        $pdf->writeHTML($html, true, false, true, false, '');
        $pdf->write2DBarcode($url_validacion_qr, 'QRCODE,L', 141, 187, 27, 27, $style, 'N');
        ob_end_clean();
    ...
    function getBodyTramite2($html, $tramite, $titularTramite , $constanciaNoVotacion) {
        $paisModel = new PaisModel();
        ...
        $provincia = null;
        $provinciaModel = new ProvinciaModel();  
        if($constanciaNoVotacion != null) {
            $provincia =  $provinciaModel->where('id_provincia', $constanciaNoVotacion['id_provincia'])->first();
        }


        
        $razonSocialKey = $constanciaNoVotacion['razon_social'];
        $razonSocialValue ="";
        $estadosRazonSocial = $this->getRazonesNoVotacion();
        foreach($estadosRazonSocial as $key => $value) {
           if ($key == $razonSocialKey) {
              $razonSocialValue = $value;
           }   
        }
        $nombreEleccion = $constanciaNoVotacion['nombre_eleccion'];
        /*$nombreEleccion = "";
        if (isset($constanciaNoVotacion['nombre_eleccion'])) {
            $nombreEleccion = $this->getTipoEleccion($constanciaNoVotacion['nombre_eleccion']);
        }*/
        ...
        $html = $html . '<table>
            <tr>
                <td width="100%" align="center">
                    <br/><br/>
                    <font size="14"><b>EXPOSICION POLICIAL (NO VOTACION)</b></font>
                </td>
            </tr>
            <tr>
...
documento, según datos aportados por el solicitante:
...
<br/><br/>
Quien Manifiesta que <strong>NO PODRA EMITIR SU VOTO</strong> en las '.$nombreEleccion.', de fecha <strong>'.date_format(date_create($constanciaNoVotacion['fecha_votacion']), 'd/m/Y').'</strong>
por la siguiente razón : 
<br/>
<table border="0">
    <tr>
        <td width="33%" bgcolor="#c3c3c3">MOTIVO :</td>
        <td width="67%" colspan="3">&nbsp;&nbsp;<b>' . $razonSocialValue . '.</b> '. $constanciaNoVotacion['descripcion'].'</td>
    </tr>
   
</table>
<br/><br/>
Para ser presentando ante las Autoridades de la Justicia Electoral de la Provincia de '.$provincia['nombre'].'.
<br/><br/>
...
<br/><br/>
Este instrumento es a título gratuito en la franja horaria de 06:00 a 18:00 hs. del día 13/08/2023
conforme resolución emitida por Jefatura de la Policía - Res. Pol.(900-JP/23)

<br/><br/>
DOCUMENTO RESPALDADO MEDIANTE<br/>RESOLUCION MINISTERIAL 294-MS/23
    ...
    public function guardarFileDisk($id_tramite) {
        ...
        $titularTramite = $tramitePersonaModel->where('id_tramite', $tramite['id_tramite'])->where('es_titular_tramite', INT_UNO)->first();
        $tramiteNoVotacionModel = new TramiteVotacionModel();
        $constanciaNoVotacion = $tramiteNoVotacionModel->where('id_tramite', $id_tramite)->first();
