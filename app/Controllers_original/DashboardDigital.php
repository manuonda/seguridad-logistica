...
class DashboardDigital extends BaseController
{
...
// 		var_dump($cantidad);
	...
	private function get_format_row($tramite)
	{
		...
		$row = '<tr>' .
		    ...
		    if ($tramite['estado_pago'] != APROBADO || empty($tramite['estado_pago'])) {
	...
	public function sendEmail($idTramite) {
	  ...
	  $status = $email->sendEmail($remitente, $titular_tramite['email'],$subject ,$filePath,"Estimado/a: A continuación, se adjunta el comprobante del trámite."); 
	...
	public function mostrarFirmaDigital() {
		...
		try{
		  if ($archivoTramiteDigital != null && 
			  ...
			  file_exists($archivoTramiteDigital['ruta'])) {
			  $status ="OK";                         
		  } else {
              $status = "ERROR";
		  }
	...
	public function descargarFirmaDigital() {
	   ...
	   if( $archivoTramiteDigital != null && 
		   ...
		   header('Content-Length: ' . filesize($archivoTramiteDigital['ruta']));
		   flush(); // Flush system output buffer
		   readfile($archivoTramiteDigital['ruta']);
	 	   //die();
	  } else {
